...
def run_debate_on_question(item, index):
    ...
    for turn in range(1, MAX_TURNS + 1):
        ...
        # --- GENERATOR ---
        gen_sys = "You are a physics expert. Solve step-by-step. You MUST end your response with the final multiple choice letter inside brackets, like [A], [B], [C], or [D]."
        if turn == 1:
        ...
        # --- VERIFIER ---
        ver_sys = (
            "You are a strict physics auditor. Check the assistant's work. "
            "If the physics reasoning and final letter are 100% correct, reply EXACTLY and ONLY with the word 'VERIFIED'. "
            "If it is wrong, DO NOT use the word 'VERIFIED'. Instead, explain the specific error."
        )
        ver_prompt = f"Question: {question}\nChoices:\n{choices_text}\nAssistant Answer: {current_answer}\nIs the logic and final letter correct?"
...
def run_benchmark():
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write(f"LLM DEBATE EVALUATOR LOG - PHYSICS\n")
        f.write(f"Started at: {timestamp}\n")
        f.write(f"Dataset: MMLU High School Physics | Gen: {GENERATOR_MODEL} | Ver: {VERIFIER_MODEL}\n")
        f.write("="*60 + "\n")

    print("="*60)
    print("🚀 MEMULAI BENCHMARK EVALUATOR - FISIKA (MULTI-AGENT)")
    print(f"Dataset: MMLU High School Physics | Gen: {GENERATOR_MODEL} | Ver: {VERIFIER_MODEL}")
    print("="*60)
